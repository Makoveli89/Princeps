...
class PlanStep:
    ...
    estimated_effort: str | None = None  # e.g., "low", "medium", "high"
...
class PlannerAgent(BaseAgent):
    ...
    def __init__(
        ...
        logger.info(
            f"PlannerAgent '{agent_name}' initialized with {len(self.council.members)} council members"
        )
...
""",
        ...
        prompt = (
            base_prompt + mode_specific.get(mode, "") + "\n" + plan_type_context.get(plan_type, "")
        )
    ...
    def _process_response(
        ...
        plan = ExecutionPlan(
            ...
            steps=steps,
            planning_mode=PlanningMode(
                task.parameters.get("planning_mode", PlanningMode.DECOMPOSE.value)
            ),
            reasoning=reasoning,
            risks=risks,
    ...
    async def execute_task(self, task: AgentTask) -> AgentResponse:
        ...
        use_council = (
            self.planner_config.enable_council
            and len(self.council.members) >= self.planner_config.min_council_members
            and task.parameters.get("use_council", True)
        )
    ...
    async def _execute_with_council(self, task: AgentTask) -> AgentResponse:
        ...
        try:
            ...
            if (
                execution_plan.quality_assessment.value
                < self.planner_config.min_quality_threshold.value
                and self.planner_config.retry_on_low_quality
            ):
                self._log_event(
                    "low_quality_plan",
                    {
            ...
            response = AgentResponse(
                ...
                llm_provider=LLMProvider(decision.winning_proposal.member.provider.upper())
                if hasattr(LLMProvider, decision.winning_proposal.member.provider.upper())
                else None,
    ...
    def _update_planning_stats(self, plan: ExecutionPlan, decision: CouncilDecision):
        ...
        self.planning_stats["avg_agreement_score"] = (
            old_avg * (n - 1) + decision.agreement_score
        ) / n
