...
def get_run_history(session):
    ...
    runs = (
        session.query(AgentRun.started_at, AgentRun.success, AgentRun.agent_id)
        .filter(AgentRun.tenant_id == ws_id, AgentRun.started_at >= start_date)
        .all()
    )
...
def get_recent_failures(session):
    failures = (
        session.query(AgentRun)
        .filter(AgentRun.tenant_id == ws_id, not AgentRun.success)
    ...
    return [
        {"id": str(r.id), "time": r.started_at, "agent": r.agent_id, "task": r.task[:100]}
        for r in failures
    ]
...
if failures:
    for f in failures:
        st.error(
            f"**{f['time']}** | Agent: {f['agent']} | Task: {f['task']}... [Run ID: {f['id']}]"
        )
else:
