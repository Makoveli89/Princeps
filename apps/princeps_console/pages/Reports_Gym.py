...
with tab1:
    ...
    def get_agent_metrics(session):
        ...
        stats = (
            session.query(
                ...
                func.sum(func.cast(AgentRun.success, st.session_state.get("int_type", int))).label(
                    "success_count"
                ),  # Cast bool to int for sum
                func.avg(AgentRun.duration_ms).label("avg_latency"),
            )
            .filter(AgentRun.tenant_id == ws_id)
    ...
    def get_metrics_pythonic(session):
        runs = (
            session.query(AgentRun.agent_id, AgentRun.success, AgentRun.duration_ms)
            .filter(AgentRun.tenant_id == ws_id)
            .all()
        )
        return runs
    ...
    if runs_data:
        ...
        st.dataframe(
            ...
            column_config={
                ...
                "Avg_Latency": st.column_config.NumberColumn("Avg Latency (ms)"),
            },
        )
    else:
...
with tab2:
    ...
    gym_questions = [
        ...
        {
            "id": "G2",
            "q": "Summarize the concept of 'idempotency'.",
            "expect": "multiple times",
        },  # loose keyword match
