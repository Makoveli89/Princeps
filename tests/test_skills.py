...
async def test_resolver_match():
    ...
    mock_llm.generate = AsyncMock(
        return_value={"text": '{"skill": "mock-skill", "parameters": {"test": "value"}}'}
    )
...
async def test_resolver_no_match():
    mock_llm = MagicMock()
    mock_llm.generate = AsyncMock(return_value={"text": '{"skill": "none"}'})
...
async def test_resolver_json_parsing_error():
    mock_llm = MagicMock()
    mock_llm.generate = AsyncMock(return_value={"text": "Invalid JSON"})
