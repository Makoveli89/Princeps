...
class DependencyParser:
    ...
    PYTHON_STDLIB = {
        ...
        'dataclasses', 'enum', 'abc', 'io', 'copy', 'pickle', 'sqlite3',
    }
    ...
    def _detect_language(self, path: str) -> str:
        ...
        return {'.py': 'python', '.js': 'javascript', '.jsx': 'javascript', '.ts': 'typescript', '.tsx': 'typescript'}.get(ext, 'unknown')
        ...
        for node in ast.walk(tree):
            if isinstance(node, ast.Import):
                ...
                dependencies.append({
                    ...
                    'is_external': module.split('.')[0] not in self.PYTHON_STDLIB if module else False,
                })
    ...
    def _parse_python_regex(self, content: str) -> list[dict[str, Any]]:
        ...
        for i, line in enumerate(content.split('\n'), 1):
            ...
            if match:
                dependencies.append({'module_name': match.group(1), 'line_number': i, 'is_relative': False, 'is_external': match.group(1).split('.')[0] not in self.PYTHON_STDLIB})
                continue
            match = from_re.match(line.strip())
            if match:
                module = match.group(2) or ''
                dependencies.append({'module_name': module, 'line_number': i, 'is_relative': len(match.group(1) or '') > 0, 'is_external': module.split('.')[0] not in self.PYTHON_STDLIB if module else False})
        ...
        for i, line in enumerate(content.split('\n'), 1):
            for pattern in [es6_re, require_re]:
                ...
                if match:
                    ...
                    dependencies.append({'module_name': module, 'line_number': i, 'is_relative': module.startswith('.'), 'is_external': not module.startswith('.')})
...
class GitUtil:
    ...
    def get_repo_info(path: str) -> dict[str, Any] | None:
        try:
            ...
            result = subprocess.run(['git', 'rev-parse', '--abbrev-ref', 'HEAD'], cwd=path, capture_output=True, text=True)
            branch = result.stdout.strip() if result.returncode == 0 else 'main'

            result = subprocess.run(['git', 'log', '-1', '--format=%H|%aI|%s'], cwd=path, capture_output=True, text=True)
            commit_info = {}
            if result.returncode == 0:
    ...
    def get_file_commit(path: str, file_path: str) -> dict[str, str] | None:
        try:
            result = subprocess.run(['git', 'log', '-1', '--format=%H|%aI|%an', '--', file_path], cwd=path, capture_output=True, text=True)
...
class IngestService:
    ...
    def __init__(self, config: IngestConfig = None, db_url: str = None):
        ...
        self.chunker = TextChunker(chunk_tokens=self.config.chunk_tokens, overlap_tokens=self.config.overlap_tokens)
    ...
    def ingest_document(self, path: str, tenant_name: str = None, source: str = "ingestion", metadata: dict[str, Any] = None) -> IngestResult:
        ...
        try:
            ...
            with get_session() as session:
                ...
                operation, created = get_or_create_operation(session, tenant_id, OperationTypeEnum.INGEST_DOCUMENT, op_inputs)
                ...
                try:
                    ...
                    if self.config.generate_embeddings and chunk_texts:
                        embeddings = self.embedder.embed(chunk_texts, batch_size=self.config.embedding_batch_size)
                    ...
                    mark_operation_success(session, operation.id, {'document_id': str(document.id), 'chunks': len(chunks)})
                    result.success = True
    ...
    def ingest_repository(self, path: str, url: str = None, tenant_name: str = None, source: str = "repo_ingestion") -> IngestResult:
        ...
        try:
            ...
            with get_session() as session:
                ...
                op_inputs = {'path': str(repo_path), 'url': url or (git_info or {}).get('remote_url')}
                operation, created = get_or_create_operation(session, tenant_id, OperationTypeEnum.INGEST_REPO, op_inputs)
                result.operation_id = str(operation.id)
                ...
                try:
                    ...
                    repository = session.query(Repository).filter_by(tenant_id=tenant_id, url=repo_url).first()
                    if not repository:
                    ...
                    if git_info and git_info.get('commit'):
                        ...
                        repository.last_commit_date = datetime.fromisoformat(commit['date']) if commit.get('date') else None
                    ...
                    for file_path in self._iter_repo_files(repo_path):
                        ...
                        try:
                            file_result = self._process_repo_file(session, tenant_id, repository, repo_path, file_path, source)
                    ...
                    mark_operation_success(session, operation.id, {'repository_id': str(repository.id), 'files': file_count})
                    result.success = True
    ...
    def _iter_repo_files(self, repo_path: Path) -> Generator[Path, None, None]:
        for file_path in repo_path.rglob('*'):
            ...
            skip = any(p in rel_path if not p.startswith('*') else rel_path.endswith(p[1:]) for p in self.config.exclude_patterns)
            if not skip:
                yield file_path

    def _process_repo_file(self, session: Session, tenant_id: str, repository: Repository, repo_path: Path, file_path: Path, source: str) -> dict[str, int]:
        ...
        existing = session.query(Resource).filter_by(tenant_id=tenant_id, repository_id=repository.id, file_path=rel_path).first()
        ...
        resource_type = {'.py': ResourceTypeEnum.CODE_FILE, '.js': ResourceTypeEnum.CODE_FILE, '.pdf': ResourceTypeEnum.DOCUMENT, '.md': ResourceTypeEnum.DOCUMENT}.get(ext, ResourceTypeEnum.CODE_FILE)

        if existing:
            resource = existing
            resource.content_hash = content_hash
        else:
            resource = Resource(tenant_id=tenant_id, repository_id=repository.id, file_path=rel_path, file_name=file_path.name, file_extension=ext, resource_type=resource_type, content_hash=content_hash)
            session.add(resource)
        ...
        if commit_info:
            ...
            resource.last_modified_at = datetime.fromisoformat(commit_info['date']) if commit_info.get('date') else None
        ...
        if self.config.parse_dependencies and ext in ['.py', '.js', '.ts', '.jsx', '.tsx']:
            for dep in self.dep_parser.parse(text, rel_path):
                session.add(ResourceDependency(tenant_id=tenant_id, source_id=resource.id, dependency_type='import', import_path=dep.get('module_name'), line_number=dep.get('line_number')))
                counters['dependencies'] += 1

        doc_type = KnowledgeTypeEnum.CODE_SNIPPET if resource_type == ResourceTypeEnum.CODE_FILE else KnowledgeTypeEnum.DOCUMENTATION
        document = Document(
            tenant_id=tenant_id,
            source_resource_id=resource.id,
        ...
        chunks = self.chunker.chunk(text)
        chunk_texts = [c['content'] for c in chunks]
        embeddings = self.embedder.embed(chunk_texts, batch_size=self.config.embedding_batch_size) if self.config.generate_embeddings and chunk_texts else []
...
def main():
    ...
    config = IngestConfig(chunk_tokens=args.chunk_tokens, generate_embeddings=not args.no_embeddings)
    ...
    print(f"Documents: {result.documents_created}, Chunks: {result.chunks_created}, Embeddings: {result.embeddings_generated}")
