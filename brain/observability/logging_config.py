...
class OperationContext:
    ...
    def __init__(
        ...
        auto_generate_operation: bool = False,
        **extra_context,
    ):
...
class StructuredFormatter(logging.Formatter):
    ...
    def format(self, record: logging.LogRecord) -> str:
        ...
        if self.include_context:
            ...
            if op_id := get_operation_id():
                log_obj["operation_id"] = op_id
            if agent_id := get_agent_id():
                log_obj["agent_id"] = agent_id
            if tenant_id := get_tenant_id():
                log_obj["tenant_id"] = tenant_id
            if extra := _extra_context.get():
...
def setup_logging(
    ...
    formatter = (
        StructuredFormatter(include_location=include_location, extra_fields=extra_fields or {})
        if json_format
    ...
    if log_to_file:
        ...
        file_handler = RotatingFileHandler(
            log_path, maxBytes=max_file_size_mb * 1024 * 1024, backupCount=backup_count
        )
...
class OperationLogger:
    ...
    def __exit__(self, exc_type, exc_val, exc_tb):
        ...
        if exc_type is None:
            self.logger.info(
                f"Completed operation: {self.operation_name} (duration: {duration_ms}ms)",
                extra={
                    "extra_data": {
            ...
            self.logger.error(
                f"Failed operation: {self.operation_name} (duration: {duration_ms}ms) - {exc_val}",
...
def log_exception(
    logger: logging.Logger, message: str, exception: Exception | None = None, **kwargs
) -> None:
    """Log an exception with context."""
    extra = {"extra_data": kwargs} if kwargs else {}
    if exception:
        logger.error(
            message, exc_info=(type(exception), exception, exception.__traceback__), extra=extra
        )
    else:
