...
class MetricsCollector:
    ...
    def collect_content_metrics(self) -> ContentMetrics:
        ...
        m.total_repositories = (
            ...
            .filter(self._tenant_filter(Repository), Repository.is_active)
            .count()
        )
        m.total_resources = (
            self.session.query(Resource)
            .filter(self._tenant_filter(Resource), Resource.is_active)
            .count()
        )
        m.total_documents = (
        ...
        m.pii_flagged_count = (
            self.session.query(Resource)
            .filter(self._tenant_filter(Resource), Resource.has_pii)
            .count()
        )
        m.secrets_flagged_count = (
            self.session.query(Resource)
            .filter(self._tenant_filter(Resource), Resource.has_secrets)
            .count()
        )
        return m
    ...
    def collect_agent_metrics(self, since: datetime | None = None) -> list[AgentMetrics]:
        ...
        query = self.session.query(
            ...
            func.sum(case((AgentRun.success, 1), else_=0)).label("success"),
            func.sum(case((not AgentRun.success, 1), else_=0)).label("failed"),
