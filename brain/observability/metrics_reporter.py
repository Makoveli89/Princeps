...
class AgentMetrics:
    ...
    def to_dict(self) -> dict[str, Any]:
        return {k: round(v, 2) if isinstance(v, float) and v is not None else v for k, v in self.__dict__.items()}
...
class MetricsCollector:
    ...
    def collect_system_metrics(self, since: datetime | None = None) -> SystemMetrics:
        ...
        avg = self.session.query(func.avg(Operation.duration_ms)).filter(
            self._tenant_filter(Operation), Operation.duration_ms.isnot(None)).scalar()
        ...
        avg_run = self.session.query(func.avg(AgentRun.duration_ms)).filter(
            self._tenant_filter(AgentRun), AgentRun.duration_ms.isnot(None)).scalar()
        metrics.avg_run_duration_ms = float(avg_run or 0)

        metrics.oldest_pending_operation = self.session.query(func.min(Operation.created_at)).filter(
            self._tenant_filter(Operation), Operation.status == OperationStatusEnum.PENDING).scalar()
        metrics.last_operation_time = self.session.query(func.max(Operation.created_at)).filter(
            self._tenant_filter(Operation)).scalar()
        metrics.last_run_time = self.session.query(func.max(AgentRun.started_at)).filter(
            self._tenant_filter(AgentRun)).scalar()
    ...
    def collect_content_metrics(self) -> ContentMetrics:
        ...
        m = ContentMetrics()
        m.total_repositories = self.session.query(Repository).filter(self._tenant_filter(Repository), Repository.is_active == True).count()
        m.total_resources = self.session.query(Resource).filter(self._tenant_filter(Resource), Resource.is_active == True).count()
        m.total_documents = self.session.query(Document).filter(self._tenant_filter(Document)).count()
        m.total_chunks = self.session.query(DocChunk).filter(self._tenant_filter(DocChunk)).count()
        m.documents_with_summaries = self.session.query(func.count(func.distinct(DocumentSummary.document_id))).filter(self._tenant_filter(DocumentSummary)).scalar() or 0
        m.documents_with_entities = self.session.query(func.count(func.distinct(DocumentEntity.document_id))).filter(self._tenant_filter(DocumentEntity)).scalar() or 0
        m.documents_with_topics = self.session.query(func.count(func.distinct(DocumentTopic.document_id))).filter(self._tenant_filter(DocumentTopic)).scalar() or 0
        m.documents_with_concepts = self.session.query(func.count(func.distinct(DocumentConcept.document_id))).filter(self._tenant_filter(DocumentConcept)).scalar() or 0
        m.total_entities = self.session.query(DocumentEntity).filter(self._tenant_filter(DocumentEntity)).count()
        m.total_topics = self.session.query(DocumentTopic).filter(self._tenant_filter(DocumentTopic)).count()
        m.total_concepts = self.session.query(DocumentConcept).filter(self._tenant_filter(DocumentConcept)).count()
        m.pii_flagged_count = self.session.query(Resource).filter(self._tenant_filter(Resource), Resource.has_pii == True).count()
        m.secrets_flagged_count = self.session.query(Resource).filter(self._tenant_filter(Resource), Resource.has_secrets == True).count()
        return m
    ...
    def collect_operation_metrics(self, since: datetime | None = None) -> list[OperationMetrics]:
        ...
        query = self.session.query(
            ...
            func.sum(case((Operation.status == OperationStatusEnum.SUCCESS, 1), else_=0)).label('success'),
            func.sum(case((Operation.status == OperationStatusEnum.FAILED, 1), else_=0)).label('failed'),
            func.sum(case((Operation.status == OperationStatusEnum.PENDING, 1), else_=0)).label('pending'),
            func.sum(case((Operation.status == OperationStatusEnum.SKIPPED, 1), else_=0)).label('skipped'),
        ...
        for row in query.group_by(Operation.op_type).all():
            ...
            results.append(OperationMetrics(
                op_type=row.op_type.value if row.op_type else "unknown",
                total_count=total, success_count=row.success or 0, failed_count=failed,
                pending_count=row.pending or 0, skipped_count=row.skipped or 0,
                avg_duration_ms=float(row.avg_duration or 0),
                min_duration_ms=row.min_duration or 0, max_duration_ms=row.max_duration or 0,
    ...
    def collect_agent_metrics(self, since: datetime | None = None) -> list[AgentMetrics]:
        ...
        if since:
            ...
            first = self.session.query(func.min(AgentRun.started_at)).filter(self._tenant_filter(AgentRun)).scalar()
            time_range_hours = max(1, (datetime.utcnow() - first).total_seconds() / 3600) if first else 1
        ...
        for row in query.group_by(AgentRun.agent_id).all():
            ...
            results.append(AgentMetrics(
                agent_id=row.agent_id, total_runs=total, successful_runs=success,
                failed_runs=row.failed or 0, avg_score=float(row.avg_score) if row.avg_score else None,
                avg_duration_ms=float(row.avg_duration or 0),
...
class MetricsReporter:
    ...
    def __init__(self, session: Session, tenant_id: str | UUID | None = None, time_range_hours: int | None = None):
        self.session = session
    ...
    def collect_all(self) -> MetricsSummary:
        """Collect all metrics."""
        since = datetime.utcnow() - timedelta(hours=self.time_range_hours) if self.time_range_hours else None
        return MetricsSummary(
            tenant_id=self.tenant_id, time_range_hours=self.time_range_hours,
    ...
    def print_report(self) -> None:
        ...
        if self.tenant_id: print(f"Tenant: {self.tenant_id}")
        if self.time_range_hours: print(f"Time Range: Last {self.time_range_hours} hours")

        print(f"\n{'-'*40}\nSYSTEM OVERVIEW\n{'-'*40}")
        sys = s.system
        print(f"  Operations: {sys.total_operations:,} total, {sys.operations_last_hour:,} (1h), {sys.operations_last_24h:,} (24h)")
        print(f"  Agent Runs: {sys.total_agent_runs:,} total, {sys.runs_last_hour:,} (1h), {sys.runs_last_24h:,} (24h)")
        print(f"  Success/Error Rate: {sys.overall_success_rate*100:.1f}% / {sys.overall_error_rate*100:.1f}%")
        print(f"  Avg Duration: {sys.avg_operation_duration_ms:.0f}ms (ops), {sys.avg_run_duration_ms:.0f}ms (runs)")

        print(f"\n{'-'*40}\nCONTENT\n{'-'*40}")
        c = s.content
        print(f"  Repos: {c.total_repositories:,} | Resources: {c.total_resources:,} | Docs: {c.total_documents:,} | Chunks: {c.total_chunks:,}")
        print(f"  Knowledge: {c.total_entities:,} entities, {c.total_topics:,} topics, {c.total_concepts:,} concepts")
        print(f"  Security: {c.pii_flagged_count:,} PII flagged, {c.secrets_flagged_count:,} secrets flagged")

        if s.operations_by_type:
            print(f"\n{'-'*40}\nOPERATIONS BY TYPE\n{'-'*40}")
            for op in s.operations_by_type:
                print(f"  {op.op_type}: {op.total_count} total, {op.success_count} success, {op.error_rate*100:.1f}% errors")

        if s.agents:
            print(f"\n{'-'*40}\nAGENTS\n{'-'*40}")
            for a in s.agents:
                print(f"  {a.agent_id}: {a.total_runs} runs, {a.success_rate*100:.1f}% success, {a.tasks_per_hour:.1f}/hour")
        print(f"\n{'='*60}\n")
...
def get_metrics_summary(session: Session, tenant_id: str | UUID | None = None, time_range_hours: int | None = None) -> MetricsSummary:
    """Get a quick metrics summary."""
    return MetricsReporter(session, tenant_id, time_range_hours).collect_all()


def print_metrics_report(session: Session, tenant_id: str | UUID | None = None, time_range_hours: int | None = None) -> None:
